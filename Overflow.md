...
### 1. 为什么要使用CAN
    ...
    2. 工业上的设备交流也是CAN通讯
### 2. CAN相关的基础知识
1. 分层思想：不同层的分工不同，抽象层级别不同
- 整体项目的架构：底层实现-中间层算法-中间层电机设备-顶层的战车业务
- CAN通信层：三层（物理信号层-链路传输层-应用数据层）
    1. 物理信号层： 规定了硬件电路的设计规则，用户使用的接线方式，线路上传输信号与电平格式的对应关系
    2. 链路传输层： 规定了地址（CANID），冲突与监测和避免（仲裁），误码校验，滤波器和掩码，帧格式
    3. 应用数据层： 规定了CANID的分配，帧格式，数据段，错误处理，帧间隔，帧的封装与解封装
2. 物理信号：接线方式-总线接法，需要在CAN_H和CAN_L之间接一个120欧姆的电阻
3. 电平格式：差分信号传输,长距离传输保证信号质量；逻辑1：电平相同，隐性电平；逻辑0：电平相反，显性电平
4. 比特率&波特率
    1. 比特率，bit/s，表示每秒传输的二进制码元的数量
    2. 波特率，symbol/s，表示每秒传输的有效码元的数量
    3. 举例：CAN通信中，比特率等于波特率，因为有效信息码元直接对应二进制码元；十六进制字符编码中，如果一秒传输1000个字符，每个字符对应一种唯一的电平信号，那么波特率就是1000symbol/s;由于一个十六进制字符要用四位二进制编码表示，那么它的比特率就是4000bit/s
5. 异步通信
    1. 把每一个码元的时间细分时间片，不同的时间片做不同的事情（同步段-传播段-相位缓冲1，相位缓冲2）
    2. 传播段：时间占用片，可能CAN总线较长，需要考虑信号在总线上的传播时间以及CAN驱动器的处理时间。传播速度一般是光速的2/3，传播时间一般是信号在总线传输时间的两倍
    4. 相位缓冲段：两个相位缓冲段之间的时间点就是 **采样时间点** ，硬件设备在这个时间点对电平进行采样，用来确定当前bit的状态是0还是1
    ![image](./image/CAN_1.png)
6. 收发模式
    1. 常规模式：向总线发送，从总线接收
    2. 回环模式：向总线和本机发送，仅从本机接收
    3. 静默模式：不向总线发送仅向本机发送，从总线和本机接收
    4. 回环静默模式： 自收自发
7. CAN_ID：标识符而非地址
8. 仲裁机制： 电平仲裁的物理特性：有显性电平（逻辑0）的时候，总线上一定是显性电平。
9. 链路传输：
    1. 滤波器：是过滤通过机制，而非过滤排除机制，即符合条件的报文会接收
    2. 匹配CANID和掩码：需要配置ID_filter和Mask，Mask的作用是判断接收id的每一位是否需要和ID_filter严格匹配，或者忽略，严格匹配的位Mask写1，允许忽略的位Mask写0，然后计算ID_recive & Mask，如果结果和ID_filter相等，则接收该报文，否则无事发生
10. 通信帧
    1. 计算机网络中：物理层成为信号，链路层称为帧，网络层称为报文，传输层叫做流，再网上叫做数据。
    2. 数据帧/遥控帧/错误帧/过载帧/帧间隔（后三者硬件设备会自动发送），帧间隔用来给自动发送帧进行插空发送
    3. 标准数据帧：
    ![image](./image/CAN_2.png)
    4. 帧仲裁：ID小的（最后发送逻辑1 隐性电平的）优先级高（先发1的会被后发1此时发送的0显性电平0覆盖）
### 3. STM32配置
    1. CAN参数配置
...
### 3.解决头文件引用问题：
- 原因：hpp需要含有class的声明给其他cpp文件使用，但是暴露c类型接口的头文件不能含有class的声明
- 解决：给每一个cpp文件新建两个头文件hpp文件用于类声明，给cpp使用；h文件用于暴露cpp接口给c文件使用
